...
if __name__ == '__main__': # makes sure that the following code is executed only if the script is run directly, not importad as a 
    ...
    sjob = openactin.SlurmJobArray("Simulations_scratch/Box_electrostatics_comparison_conditions/Run1", parameters, test_parameters) #This line creates an instance of the SlurmJobArray class from the openactin module. The constructor of the SlurmJobArray class takes four arguments: a file path "Simulations/Box/Boxv3", dictionaries parameters and test_parameters, and the job_id variable. This instance of sjob represents a job array for SLURM job submission.
    #sjob = openactin.SlurmJobArray("/Users/nusaybaelali/documents/fis/coarsegrainedactin/simulations/box/boxv6", parameters, test_parameters, job_id)
    ...
    for i in range(n_actins):
        full_model += [openactin.create_actin(length=actinLen,
